...
export class DataExplorerService {
  ...
  async getOverview(tenantId: string): Promise<ExplorerOverview> {
    ...
    const summary = {
      running: { count: 0, connectors: [] as string[] },
      completed: { count: 0, connectors: [] as string[] },
      failed: { count: 0, connectors: [] as string[] },
    }
    for (const group of syncSummary) {
      const key = group.status as keyof typeof summary
      if (key in summary) {
        summary[key].count += group._count
        if (!summary[key].connectors.includes(group.connectorType)) {
          summary[key].connectors.push(group.connectorType)
        }
      }
    }
