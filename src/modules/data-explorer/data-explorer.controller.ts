...
export class DataExplorerController {
  ...
  @Roles(UserRole.SOC_ANALYST_L1)
  async getGraylogEventDefinitions(@TenantId() tenantId: string) {
  ...
  async syncGrafanaDashboards(@TenantId() tenantId: string) {
  ...
  @Roles(UserRole.SOC_ANALYST_L1)
  async getInfluxDBBuckets(@TenantId() tenantId: string) {
  ...
  @Roles(UserRole.SOC_ANALYST_L1)
  async getMispEventDetail(@TenantId() tenantId: string, @Param('eventId') eventId: string) {
  ...
  @Roles(UserRole.TENANT_ADMIN)
  @Throttle({ default: { limit: 3, ttl: 60000 } })
  async syncVelociraptorMetadata(@TenantId() tenantId: string) {
  ...
  @Roles(UserRole.TENANT_ADMIN)
  @Throttle({ default: { limit: 3, ttl: 60000 } })
  async syncLogstashLogs(@TenantId() tenantId: string) {
  ...
  @Roles(UserRole.TENANT_ADMIN)
  @Throttle({ default: { limit: 3, ttl: 60000 } })
  async syncShuffleWorkflows(@TenantId() tenantId: string) {
