...
33. **Cross-cutting libraries MUST be modularized as shared modules** — Any library, utility, or infrastructure concern used by 3+ modules (Redis, logging, email, file storage, message queues, caching, external HTTP clients, etc.) MUST be wrapped in a dedicated shared module under `src/redis/`, `src/common/services/`, or a new top-level `src/<concern>/` directory. Each shared module provides a `@Global()` NestJS module or a plain reusable class. Never duplicate initialization, configuration, or connection logic across individual feature modules. Examples: `RedisModule` for Redis, `ServiceLogger` class for structured logging, `PrismaModule` for database. When adding a new infrastructure dependency, check if a shared module already exists before creating inline instances.
