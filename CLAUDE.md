...
- **Permission enum** (`src/common/enums/permission.enum.ts`) is the single source of truth. When adding a new permission:
  1. Add to the enum
  2. Add to `permission-definitions.ts` (with labelKey and sortOrder)
  3. Add to `default-permissions.ts` for appropriate roles
  4. Add the `@RequirePermission()` decorator to the controller endpoint
  5. Run `npx prisma db seed` to populate the database
  6. Mirror the new permission in the frontend enum
  7. Add translations for the new permission label in all 6 locale files
